...
void showNewData() {
    if (newData == true) {  // 데이터가 갱신되었다면(= 새로운 데이터가 들어왔다면)
        ...
        if (!s.equals(previousData)) {  // 현재 들어온 값이 이전 값과 동일하지 않고
          if (s.equals("0")) {  // 들어온 값이 0(이완 신호)라면
            alltorest();
            delay(5000);  // 실시간 스트림을 통해 현재와 반대되는 값을 받았을 경우, 이완되는 도중 수축되는 것을 방지하기 위한 딜레이
          }
          else if (s.equals("1")) {  // 들어온 값이 1(수축 신호)라면
            alltomax();
            delay(5000);
          }
